switch (actionSelected) do {
  case 0: {
    if !(playerSelected isEqualTo 0) then {
      [[name player,getPosATL player],{
        cutText [format["Teleporting to %1", _this select 0],"BLACK OUT"];
        sleep 3;
        player setPos ((_this select 1) findEmptyPosition [1,100,typeOf player]);
        sleep 0.5;
        cutText ["","BLACK IN"];
  ...
  case 1: {
    if !(playerSelected isEqualTo 0) then {
      ...
      sleep 3;
      player setPos ((getPosATL playerSelected) findEmptyPosition [1,100,typeOf player]);
      sleep 0.5;
      cutText ["","BLACK IN"];
    };
  };//Teleport to
  case 2: {
    [[name player,getPosATL player],{
      ...
      sleep 3;
      player setPos ((_this select 1) findEmptyPosition [2,100,typeOf player]);
      sleep 0.5;
      cutText ["","BLACK IN"];
  ...
  case 3: {
    [[name player,getPosATL player],{
      if (pRole == 0 || (player distance (_this select 1) <= 10)) exitWith {if !(pRole == 0) then {5 call TFAR_fnc_setVoiceVolume;TF_speak_volume_level = "whispering";TF_speak_volume_meters = 5;["Voice volume set to whisper"] call phx_fnc__hintThenClear;}};
