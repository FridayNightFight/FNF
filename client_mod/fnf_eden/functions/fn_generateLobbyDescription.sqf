...
_defenceNameTypes = ["def", "pro", "kep", "esc"];
...

{
	...
	switch (_moduleType) do {
		...
		case "fnf_module_escortObj": {_escortCount = _escortCount + 1;};
	};
...
if (_escortCount isNotEqualTo 0) then
{
	_outputText = _outputText + "Escort(" + str(_escortCount / 2) + "), ";
};
