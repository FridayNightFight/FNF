...
f_radios_acre2_giveRadioAction = {
	...
	[_x] spawn {
		waitUntil{time>3};
		hint format["[F3 ACRE2] Warning: No room to add radio '%1', report this to the mission maker. You now have a scroll-wheel action to get this radio.",_this select 0];
	};
	...
	_actionID = _unit addAction [format ["<t color='#3375D6'>[Radios] Give myself a %1 radio</t>",_radioName],
		 {
			 ...
			 if (_unit canAdd _radioToGive) then {
				...
				_unit removeAction (_this select 2);
				 ...
				 hint format["[F3 ACRE2] Warning: No room to add radio '%1', remove more stuff and try again",_radioToGive];
			 };
		 }
...
if (f_var_debugMode == 1) then {
	hint format["DEBUG (f\radios\acre2\acre2_init.sqf): Checking Player !Null"];
};
...
if (!isDedicated && (isNull player)) then
{
	...
	if (f_var_debugMode == 1) then {
		hint format["DEBUG (f\radios\acre2\acre2_init.sqf): Player !Null"];
	};
};
