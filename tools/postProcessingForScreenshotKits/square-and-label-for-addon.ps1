...
#   Write-Progress -Activity "Processing images for addon" -Status $folder.FullName
#   Set-Location $folder.FullName
#   $images = Get-ChildItem -File -Filter "*.paa"
#   ForEach ($image in $images) {
#     . "K:\SteamLibrary\steamapps\common\Arma 3 Tools\TexView2\Pal2PacE.exe" $image.FullName "$($image.DirectoryName)\$($image.BaseName).png"
#   }
#   ForEach ($image in $images) {
#     Remove-Item $image.FullName
#   }
