...
_createParentTask = {
	...
	//get task title
	_taskTitle = format["%1: Escort the %2", ([_module] call FNF_ClientSide_fnc_getDisplayObjNumber), _targetName];
	...
	_descriptionPointTwo = format["the %1 must be escorted to the drop off zone<br/><br/>", _targetName];
	...
	if (_objType isEqualTo "des") then
	{
		...
		_descriptionPointTwo = format["the %1 must be destroyed before it gets to the drop-off zone<br/><br/>", _targetName];
...
_createChildTaskTarget = {
	...
	//get task title
	_taskTitle = format["%1.1: Move the %2", ([_module] call FNF_ClientSide_fnc_getDisplayObjNumber), _targetName];
	if (_objType isEqualTo "des") then
	{
		_taskTitle = format["%1.1: Destroy the %2", ([_module] call FNF_ClientSide_fnc_getDisplayObjNumber), _targetName];
	};
	if (_customTitle isNotEqualTo "") then
	{
		_taskTitle = _customTitle;
	...
	_descriptionPointTwo = format["the %1 must be picked up and moved<br/><br/>", _targetName];
...
_createChildTaskDropOff = {
	...
	//get task title
	_taskTitle = format["%1.2: Escort the %2 to the drop-off", ([_module] call FNF_ClientSide_fnc_getDisplayObjNumber), _targetName];
	if (_objType isEqualTo "des") then
	{
