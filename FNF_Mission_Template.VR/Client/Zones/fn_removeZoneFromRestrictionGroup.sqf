...
if (_groupIndex isNotEqualTo -1 and _zoneIndex isNotEqualTo -1) then
{
	...
	if (fnf_zoneRestrictionGroupsList select _groupIndex select 3) then
	{
		[{
			...
			{
				fnf_zoneRestrictionsLastKnownPosition = ((fnf_zoneRestrictionGroupsList select _groupIndex select 1 select 0 select 0) call FNF_ClientSide_fnc_getVisualCenter);
			};
