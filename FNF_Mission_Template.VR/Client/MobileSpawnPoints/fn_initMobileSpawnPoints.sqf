...
_globalCounter = 0;
{
...

	{
		_currentPole = _x;
		{
			...
			_markerstr = createMarkerLocal ["fnf_mspMarkerName_" + str(_globalCounter),_x];
			...
			_action = ["fnf_teleAction_" + str(_globalCounter), "Teleport to MSP " + str(_globalCounter), "", {
				...
				params ["_target", "_player", "_params"];
				_vic = _params select 0;
				_distanceForDisable = _params select 1;
				_result = true;
				...
				_closePlayers = allPlayers select {(_x distance2D _vic) < _distanceForDisable and not ([playerSide, (side _x)] call BIS_fnc_sideIsFriendly)};
				if ((count _closePlayers) isNotEqualTo 0) then
				{
					_result = false;
				};
				_result;
			}, {}, [_x, _distanceForDisable]] call ace_interact_menu_fnc_createAction;
