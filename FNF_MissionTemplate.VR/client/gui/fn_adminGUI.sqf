phx_adminGUI = findDisplay 60000 createDisplay "RscDisplayEmpty";

_background = phx_adminGUI ctrlCreate ["IGUIBack", -1];
_background ctrlSetPosition [0.25,0,0.5,0.83];
_background ctrlSetBackgroundColor [0,0,0,0.5];
_background ctrlCommit 0;

_essButton = phx_adminGUI ctrlCreate ["RscButton", -1]; 
...
_mssButton buttonSetAction 'if !(f_var_mission_timer <= 1) then {
...
_pssButton buttonSetAction 'f_var_mission_timer = f_var_mission_timer + 1; publicVariableServer ""f_var_mission_timer""; systemChat format [""New safe start time: %1 mins"", f_var_mission_timer];';
...
_btwButton buttonSetAction 'phx_gameEnd = true; publicVariable ""phx_gameEnd""; west call PHX_fnc_titanMultiRoundHandler;';
_btwButton ctrlCommit 0;

_rtwButton = phx_adminGUI ctrlCreate ["RscButton", -1]; 
_rtwButton ctrlSetPosition [0.275,0.03 + 0.08 * 4,0.45,0.05];
_rtwButton ctrlSetText ("Red Team Wins");
_rtwButton buttonSetAction 'phx_gameEnd = true; publicVariable ""phx_gameEnd""; east call PHX_fnc_titanMultiRoundHandler;';
_rtwButton ctrlCommit 0;

_btpButton = phx_adminGUI ctrlCreate ["RscButton", -1]; 
_btpButton ctrlSetPosition [0.275,0.03 + 0.08 * 5,0.45,0.05];
_btpButton ctrlSetText ("Blue Team +1");
_btpButton buttonSetAction 'phx_blueTeamScore = phx_blueTeamScore + 1';
_btpButton ctrlCommit 0;

_btmButton = phx_adminGUI ctrlCreate ["RscButton", -1]; 
_btmButton ctrlSetPosition [0.275,0.03 + 0.08 * 6,0.45,0.05];
_btmButton ctrlSetText ("Blue Team -1");
_btmButton buttonSetAction 'phx_blueTeamScore = phx_blueTeamScore - 1';
_btmButton ctrlCommit 0;

_rtpButton = phx_adminGUI ctrlCreate ["RscButton", -1]; 
_rtpButton ctrlSetPosition [0.275,0.03 + 0.08 * 7,0.45,0.05];
_rtpButton ctrlSetText ("Red Team +1");
_rtpButton buttonSetAction 'phx_blueTeamScore = phx_redTeamScore + 1';
_rtpButton ctrlCommit 0;

_rtmButton = phx_adminGUI ctrlCreate ["RscButton", -1]; 
_rtmButton ctrlSetPosition [0.275,0.03 + 0.08 * 8,0.45,0.05];
_rtmButton ctrlSetText ("Red Team -1");
_rtmButton buttonSetAction 'phx_blueTeamScore = phx_redTeamScore - 1';
_rtmButton ctrlCommit 0;

_emnButton = phx_adminGUI ctrlCreate ["RscButton", -1]; 
_emnButton ctrlSetPosition [0.275,0.03 + 0.08 * 9,0.45,0.05];
_emnButton ctrlSetText ("End Mission");
_emnButton buttonSetAction '""end1"" call bis_fnc_endmissionserver;';
