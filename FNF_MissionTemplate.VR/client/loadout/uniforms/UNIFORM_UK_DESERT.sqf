...
phx_loadout_uniform  = "U_mas_ukl_B_CombatUniform_aor1";
phx_loadout_vest     = "V_mas_ukl_PlateCarrierRF_tan";
phx_loadout_backpack = "B_Kitbag_cbr";
phx_loadout_headgear = "H_mas_ukl_opscore_gog_aor1";

if (pRole == ROLE_PL || pRole == ROLE_SL || pRole == ROLE_TL || pRole == ROLE_MGTL) then {phx_loadout_vest = "V_mas_ukl_PlateCarrier1_tan";}; //PL, SL, TL, MGTL

if (pRole == ROLE_AR || pRole == ROLE_MG) then {phx_loadout_vest = "V_mas_ukl_PlateCarrierAR_tan";}; //AR & MG

if (pRole == ROLE_CR) then { phx_loadout_headgear = "H_HelmetCrew_I"; phx_loadout_vest = "V_mas_ukl_PlateCarrierLR_tan";}; // Crewman

if (pRole == ROLE_P) then { phx_loadout_headgear = "H_CrewHelmetHeli_B"; phx_loadout_vest = "V_mas_ukl_PlateCarrierLR_tan"; phx_loadout_backpack = phx_loadout_lrRadio;}; // Pilot Loadout
