...
phx_loadout_uniform  = "U_mas_ukl_B_CombatUniform_multi";
phx_loadout_vest     = "V_mas_ukl_PlateCarrierRF_rgr";
phx_loadout_backpack = "B_Kitbag_rgr";
phx_loadout_headgear = "H_mas_ukl_opscorn_gog";

if (pRole == ROLE_PL || pRole == ROLE_SL || pRole == ROLE_TL || pRole == ROLE_MGTL) then {phx_loadout_vest = "V_mas_ukl_PlateCarrier1_rgr";}; //PL, SL, TL, MGTL

if (pRole == ROLE_AR || pRole == ROLE_MG) then {phx_loadout_vest = "V_mas_ukl_PlateCarrierAR_rgr";}; //AR & MG

if (pRole == ROLE_CR) then { phx_loadout_headgear = "H_HelmetCrew_I"; phx_loadout_vest = "V_mas_ukl_PlateCarrierLR_rgr";}; // Crewman

if (pRole == ROLE_P) then { phx_loadout_headgear = "H_CrewHelmetHeli_B"; phx_loadout_vest = "V_mas_ukl_PlateCarrierLR_rgr"; phx_loadout_backpack = phx_loadout_lrRadio;}; // Pilot Loadout
