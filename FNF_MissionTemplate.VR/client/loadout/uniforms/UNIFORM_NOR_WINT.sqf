...
phx_loadout_uniform  = "U_mas_nor_B_CombatUniform_wint2";
phx_loadout_vest     = "V_mas_nor_PlateCarrierRF_wint";
phx_loadout_backpack = "B_mas_nor_Kitbag_w";
phx_loadout_headgear = "H_mas_nor_opscore_gog_w";

if (pRole == ROLE_PL || pRole == ROLE_SL || pRole == ROLE_TL || pRole == ROLE_MGTL) then {phx_loadout_vest = "V_mas_nor_PlateCarrier1_wint";}; //PL, SL, TL, MGTL

if (pRole == ROLE_AR || pRole == ROLE_MG) then {phx_loadout_vest = "V_mas_nor_PlateCarrierAR_wint";}; //AR & MG

if (pRole == ROLE_CR) then { phx_loadout_headgear = "H_HelmetCrew_I"; phx_loadout_vest = "V_mas_nor_PlateCarrierLR_wint";}; // Crewman

if (pRole == ROLE_P) then { phx_loadout_headgear = "H_CrewHelmetHeli_B"; phx_loadout_vest = "V_mas_nor_PlateCarrierLR_wint"; phx_loadout_backpack = phx_loadout_lrRadio;}; // Pilot Loadout
