...
switch (typeName _terminalHackTime) do {
  ...
  case "SCALAR": {
    ...
    if (!isNull term3) then {fnf_term3Time = _terminalHackTime};
  };
  default {
...
_terminalHacking = {
  ...
  switch (_term) do {
    case term1: {
      while {fnf_term1Time > 0 && fnf_term1Hacking} do {
        if (fnf_term1Time % 5 == 0 || fnf_term1Time <= 10) then {
          _termTimeText = "Terminal 1 hack timer: " + ([fnf_term1Time,"MM:SS"] call BIS_fnc_secondsToString);
        ...
        if (fnf_term1Hacking) then {
          ...
          _timer = fnf_term1Time;
        };
      };
    };
    case term2: {
      while {fnf_term2Time > 0 && fnf_term2Hacking} do {
        if (fnf_term2Time % 5 == 0 || fnf_term2Time <= 10) then {
          _termTimeText = "Terminal 2 hack timer: " + ([fnf_term2Time,"MM:SS"] call BIS_fnc_secondsToString);
          _termTimeText remoteExec ["hintSilent",0,false];
        ...
        if (fnf_term2Hacking) then {
          ...
          _timer = fnf_term2Time;
        };
      };
    };
    case term3: {
      while {fnf_term3Time > 0 && fnf_term3Hacking} do {
        if (fnf_term3Time % 5 == 0 || fnf_term3Time <= 10) then {
          _termTimeText = "Terminal 3 hack timer: " + ([fnf_term3Time,"MM:SS"] call BIS_fnc_secondsToString);
          _termTimeText remoteExec ["hintSilent",0,false];
...
fnf_serverTerminalAction = {
  ...
  switch (_hacking) do {
    case true: {
      switch (_term) do {
        case term1: {missionNamespace setVariable ["fnf_term1Hacking", true, true];};
        case term2: {missionNamespace setVariable ["fnf_term2Hacking", true, true];};
        case term3: {missionNamespace setVariable ["fnf_term3Hacking", true, true];};
      };
    ...
    case false: {
      switch (_term) do {
        case term1: {missionNamespace setVariable ["fnf_term1Hacking", false, true];};
        case term2: {missionNamespace setVariable ["fnf_term2Hacking", false, true];};
        case term3: {missionNamespace setVariable ["fnf_term3Hacking", false, true];};
      };

      [{
        [format ["%1 hack paused!", _this], "warning", 7] remoteExec ["fnf_ui_fnc_notify", 0]
...
_win = {
  ...
  [fnf_attackingSide, "has successfully hacked all terminals and won!"] spawn fnf_server_fnc_gameEnd;
};

[fnf_defendingSide,"defendTask1",[format ["%1 second hack time",fnf_term1Time],"Defend the data terminal","term1Mark"],term1,"AUTOASSIGNED"] call BIS_fnc_taskCreate;
[fnf_attackingSide,"attackTask1",[format ["%1 second hack time",fnf_term1Time],"Hack the data terminal","term1Mark"],term1,"AUTOASSIGNED"] call BIS_fnc_taskCreate;

waitUntil {fnf_term1Hacking};
[term1, fnf_term1Time] call _terminalHacking;

sleep 1;

[fnf_defendingSide,"defendTask2",[format ["%1 second hack time",fnf_term2Time],"Defend the data terminal","term2Mark"],term2,"AUTOASSIGNED"] call BIS_fnc_taskCreate;
[fnf_attackingSide,"attackTask2",[format ["%1 second hack time",fnf_term2Time],"Hack the data terminal","term2Mark"],term2,"AUTOASSIGNED"] call BIS_fnc_taskCreate;
"term2Mark" setMarkerText "Terminal 2 - Active";

waitUntil {fnf_term2Hacking};
[term2, fnf_term2Time] call _terminalHacking;

sleep 1;

if (!isNull term3) then {
  [fnf_defendingSide,"defendTask3",[format ["%1 second hack time",fnf_term3Time],"Defend the data terminal","term3Mark"],term3,"AUTOASSIGNED"] call BIS_fnc_taskCreate;
  [fnf_attackingSide,"attackTask3",[format ["%1 second hack time",fnf_term3Time],"Hack the data terminal","term3Mark"],term3,"AUTOASSIGNED"] call BIS_fnc_taskCreate;
  "term3Mark" setMarkerText "Terminal 3 - Active";

  waitUntil {fnf_term3Hacking};
  [term3, fnf_term3Time] call _terminalHacking;
