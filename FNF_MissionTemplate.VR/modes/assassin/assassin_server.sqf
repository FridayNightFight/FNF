...
fnf_assassinationTargets = _targets;
fnf_aliveHVTs = count fnf_assassinationTargets;
fnf_requiredKills = _requiredKills;
...
#define GETTARGET ([fnf_assassinationTargets, format["Target %1", _forEachIndex + 1]] call BIS_fnc_getFromPairs)
...
_fnc_gatherObjects = {
  ...
  if (_useDefaultPLTHQ) then {
    private "_HVTGroup";
    _HVTGroups = allGroups select {side _x isEqualTo fnf_defendingSide && groupId _x isEqualTo "PLTHQ"};
    if (count _HVTGroups > 0) then {
      _HVTGroup = _HVTGroups # 0
    } else {
      if (true) exitWith {
        [
          format["<t align='center'>Unable to locate Platoon HQ on %1, this mission will not work.</t>", fnf_defendingSide call BIS_fnc_sideName],
          "error",
          15
        ] remoteExec ["fnf_ui_fnc_notify", 0];
      };
...

    {
      ...
      [fnf_assassinationTargets, format["Target %1", _forEachIndex + 1], _arr] call BIS_fnc_setToPairs;
    } forEach (units _HVTGroup);

    // fnf_aliveHVTs = count (units _HVTGroup);
  ...
  } else {
    {
      ...
      if (!isNil HVTXVAR) then {
        [{!isNull (_this # 2)},{
          ...
          private _arr = [fnf_assassinationTargets, format["Target %1", _targetNum]] call BIS_fnc_getFromPairs;
          _arr pushBackUnique _unit;
          [fnf_assassinationTargets, format["Target %1", _targetNum], _arr] call BIS_fnc_setToPairs;
  ...
  publicVariable "fnf_assassinationTargets";
  [{
...
// (fnf_assassinationTargets select {count (_x # 1) > 2});
...
[{
  private _goodLocations = ["fnf_assassin_boundaries_"] call BIS_fnc_getMarkers;
  {
    ...
    switch (_unit getVariable ["fnf_inZone", true]) do {
      case true: {
...
[{fnf_aliveHVTs <= (count (fnf_assassinationTargets) - fnf_requiredKills)},{
  [format["<t align='center'>The required number of HVTs has been eliminated!<br/>(%1 / %2)<br/><br/>As this may be due to disconnects or other circumstances, keep fighting until the staff declare a win!</t>", fnf_requiredKills, count (fnf_assassinationTargets)],"info",15] remoteExec ["fnf_ui_fnc_notify", 0];
